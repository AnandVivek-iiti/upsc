...
const evaluateAnswer = async (req, res, next) => {
  try {
    ...
    // image is expected as { data: "data:image/jpeg;base64,...." }
    // ─────────────────────────────────────────────────────────────────────
    if (image && image.data) {
      ...
      if (!parsedImage) {
        return res.status(400).json({
          success: false,
          error: "Invalid image data. Please re-upload the photo.",
        });
      }
      ...
      console.log(
        `[Evaluation:Image] Processing for user: ${req.user.id} (${(byteSize / (1024 * 1024)).toFixed(2)}MB, ${mimeType})`,
      );
    ...
    // ─────────────────────────────────────────────────────────────────────
    // TYPED ANSWER FLOW (existing behaviour, unchanged)
    // ─────────────────────────────────────────────────────────────────────
...
const explainPrelimQuestion = async (req, res, next) => {
  try {
    ...
    if (!correctOption) {
      return res.status(400).json({
        success: false,
        error: "correctOption is required.",
      });
    }
    ...
    const hintLine = explanation
      ? `\nHint context (use only as grounding, not verbatim): ${explanation.trim()}`
      : "";

    const userPrompt = `Question: ${questionText.trim()}\n\nOptions:\n${optionsText}\n\nCorrect Answer: ${correctOption}${hintLine}`;

    console.log(`[Prelim Explain] Processing for user: ${req.user.id}`);

    const { result: explanationText, provider } = await runWithProviders(
      userPrompt,
      PRELIM_EXPLAIN_SYSTEM_INSTRUCTION,
      { mode: "text", label: "Prelim Explain", minChars: 60 },
    );
