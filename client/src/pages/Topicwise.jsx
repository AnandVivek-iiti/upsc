...
// ─── QUESTION CARD ────────────────────────────────────────────────────────────
function QuestionCard({ q, index, accentColor, revQueue, subjectMeta, onCorrect }) {
  ...
  return (
    ...
    >
      ...
      <div style={{padding:"16px 20px 12px", borderBottom:"0.5px solid var(--bg-border)", display:"flex", alignItems:"flex-start", gap:12}}>
        <div style={{fontSize:12, fontWeight:700, color:accentColor, fontFamily:"'DM Mono', monospace", minWidth:28, paddingTop:2}}>
          Q{index+1}
        </div>
        <div style={{flex:1}}>
          {(() => {
      ...
      <div style={{padding:"14px 20px"}}>
        ...
        if(!isRevealed) {
          setSelected(opt.id);
          if (opt.id === q.correctOption && onCorrect) onCorrect(q.topic, q.subTopic);
        }
...
// ─── SUBJECT PANEL ────────────────────────────────────────────────────────────
function SubjectPanel({ subject, subjectKey, accentColor, revQueue, onTopicComplete, paperLabel }) {
  ...
  const filtered = useMemo(()=>{
    return rawData.filter(q=>{
      ...
      if (showOnlyPinned && !revQueue.isPinned(q._id||q.id)) return false;
      if (search.trim()) {
  ...
  const pinnedCount= rawData.filter(q=>revQueue.isPinned(q._id||q.id)).length;
  ...
  const completedTopics = useMemo(() => {
    const done = new Set();
    rawData.forEach(q => { if (revQueue.isPinned(q._id||q.id) || q.difficulty === "Easy") done.add(q.topic); });
    return done;
  }, [rawData, revQueue]);
    ...
    <div>
      ...
      {filtered.length===0 ? (
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {filtered.map((q,i)=>(
            <QuestionCard key={q._id||i} q={q} index={i} accentColor={accentColor} revQueue={revQueue} subjectMeta={subjectMeta}
...
// ─── PAPER ACCORDION ─────────────────────────────────────────────────────────
function PaperSection({ paperId, paper, isOpen, onToggle, revQueue, onTopicComplete }) {
        ...
        <>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            {paperOrder.map(paperId=>{
              ...
              return (
                <PaperSection key={paperId} paperId={paperId} paper={paper}
