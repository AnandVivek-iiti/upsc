...
const NotesSignInRequired = memo(function NotesSignInRequired() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-6 gap-4">
      <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-[var(--accent-blue-dim)] text-[var(--accent-blue)]">
        <LogIn size={22} />
      </div>
      <div>
        <p className="text-[15px] font-bold text-[var(--text-primary)]">Sign in to use Notes</p>
        <p className="text-[13px] text-[var(--text-muted)] mt-2 max-w-[300px] leading-relaxed">
          Notes are saved to your account and visible only to you. Sign in to start writing.
        </p>
      </div>
...
const NotesLoadingState = memo(function NotesLoadingState() {
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-3">
      <Loader2 size={22} className="animate-spin text-[var(--text-muted)]" />
      <p className="text-[12.5px] font-mono text-[var(--text-muted)]">Loading your notes…</p>
    </div>
  );
});

const NotesErrorState = memo(function NotesErrorState({ message, onRetry }) {
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center px-6 gap-4">
      <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-[rgba(239,68,68,.1)] text-[#f87171]">
        <AlertTriangle size={22} />
      </div>
      <div>
        <p className="text-[15px] font-bold text-[var(--text-primary)]">Couldn't load your notes</p>
        <p className="text-[13px] text-[var(--text-muted)] mt-2 max-w-[300px] leading-relaxed">{message}</p>
      </div>
...
const EmptyNotesList = memo(function EmptyNotesList({ onCreate, filtered, creating }) {
  return (
    <div className="flex flex-col items-center justify-center text-center h-full p-8 gap-4">
      ...
      {!filtered && (
        <button onClick={onCreate} disabled={creating} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[12.5px] font-semibold bg-[var(--accent-blue)] text-white border-0 min-h-[40px] transition-all active:scale-95 disabled:opacity-50">
...
const EmptyEditor = memo(function EmptyEditor({ onCreate, creating }) {
  return (
    <div className="flex flex-col items-center justify-center text-center h-full p-8 gap-4">
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center border border-[rgba(245,158,11,0.2)]"
        style={{ background:"linear-gradient(135deg,var(--accent-gold-dim),rgba(167,139,250,.12))" }}>
      ...
      <button onClick={onCreate} disabled={creating}
        className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-[13px] font-semibold bg-[var(--accent-blue)] text-white border-0 min-h-[42px] transition-all active:scale-95 disabled:opacity-50">
        {creating ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />} Write your first note
      </button>
    </div>
  );
});
    ...
    <div className={`${paneVisible?"flex":"hidden"} lg:flex w-full lg:w-[min(320px,30vw)] xl:w-[340px] shrink-0 flex-col overflow-hidden border-r border-[var(--bg-border)] bg-[var(--bg-surface)]`}>
      <div className="p-3 border-b border-[var(--bg-border)] shrink-0 space-y-2.5">
        <div className="flex items-center justify-between gap-2">
          ...
          <button onClick={onCreate} disabled={creating}
            className="inline-flex items-center justify-center w-[36px] h-[36px] rounded-[11px] bg-[var(--accent-gold-dim)] text-[var(--accent-gold)] border-0 cursor-pointer hover:opacity-90 transition-opacity disabled:opacity-50"
            aria-label="New note">
      ...
      <div className="fixed z-[9991] bg-[var(--bg-surface)] flex flex-col overflow-hidden border border-[var(--bg-border)] rounded-[22px] shadow-lg-brand
        ...
        <div className="flex flex-col items-center text-center gap-3 py-8 px-4">
          <p className="text-[13px] text-[var(--text-secondary)] max-w-[300px]">
            Notes are saved to your account and visible only to you. Sign in to let your AI mentor improve, audit and reformat them.
          </p>
          <div className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-[11px] font-mono bg-[var(--accent-blue-dim)] text-[var(--accent-blue)]">
            <LogIn size={12} /> Tap your profile to sign in
          </div>
