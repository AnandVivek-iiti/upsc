...
import {
  ArrowUp, Cpu, MessageSquare, Sparkles, LogIn, User, Bot,
  Plus, Trash2, Brain, X, Menu,
} from "lucide-react";
import {
  chatWithMentor,
  listChatThreads,
  ...
  return (
    <div
      className={`group relative flex items-start gap-2 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-150 ${
        active ? "border border-accent-gold/25" : "hover:bg-bg-muted border border-transparent"
      }`}
    ...
    >
      ...
      {confirmDelete ? (
        <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
          <button className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-red-500/20 text-red-400 hover:bg-red-500/30"
            onClick={() => { onDelete(thread.id); setConfirmDelete(false); }}>del</button>
        ...
        <button className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-red-500/10"
...
// Props:
//   isLoggedIn     – bool
//   compact        – render small embedded widget
//   contextHint    – string hint for compact mode
//   prefill        – string to pre-fill textarea (from quote click in HeroBanner)
//   onClearPrefill – callback to clear prefill after use
export default function AIMentorChat({
  ...
  if (compact) return <CompactChat contextHint={contextHint} isLoggedIn={isLoggedIn} />;
  return (
    <FullChat
...
function CompactChat({ contextHint, isLoggedIn }) {
  ...
  return (
    <div className="glass-panel overflow-hidden">
      <button className="w-full flex items-center justify-between px-4 py-3 border-b border-bg-border"
        onClick={() => setOpen(v => !v)}>
        ...
        <>
                ...
                <div className="flex flex-wrap gap-1.5 justify-center">
                  {STARTER_QUESTIONS.slice(0, 3).map((q, i) => (
                    <button key={i} onClick={() => send(q)}
                      ...
                      style={{ borderColor: "var(--bg-border)", color: "var(--text-muted)" }}>{q}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="border-t border-bg-border p-3 flex gap-2">
            <textarea value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="Ask your UPSC mentor…" rows={1}
...
// ─── Full-page chat ───────────────────────────────────────────────────────────
function FullChat({ contextHint, isLoggedIn, prefill, onClearPrefill }) {
  ...
  // ── Apply prefill from HeroBanner quote click ─────────────────────────────
  useEffect(() => {
  ...
  const openThread = useCallback(async (id) => {
    ...
    setActiveThreadId(id); setMessages([]); setError(null); setSidebarOpen(false);
  ...
  const handleDelete = useCallback(async (id) => {
    try {
      ...
      if (id === activeThreadId) { setActiveThreadId(null); setMessages([]); setActiveTitle(""); }
    } catch { /* ignore */ }
  }, [activeThreadId]);

  const newChat = () => {
    setActiveThreadId(null); setMessages([]); setActiveTitle(""); setError(null); setInput("");
  ...
  const send = useCallback(async (text) => {
    ...
    setSending(true);
    try {
      const res = await chatWithMentor({ message: msg, contextHint, threadId: activeThreadId });
      if (!activeThreadId) {
        ...
        setThreads(prev => prev.map(t =>
          t.id === res.thread_id ? { ...t, title: res.title, updatedAt: new Date().toISOString() } : t
        ));
      }
  ...
  return (
    <div className="flex h-[calc(100vh-56px)] lg:h-screen overflow-hidden" style={{ background: "var(--bg-base)" }}>
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-20 bg-black/50 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />
      )}
      ...
      <aside className={`fixed lg:relative top-0 left-0 h-full z-30 flex flex-col shrink-0 border-r border-bg-border bg-bg-surface transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
        ...
        <div className="px-3 pt-4 pb-3 border-b border-bg-border shrink-0">
          <div className="flex items-center gap-2 mb-3">
            ...
            <button className="lg:hidden p-1 rounded-lg hover:bg-bg-muted text-text-muted" onClick={() => setSidebarOpen(false)}>
              <X size={14} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
          {threadsLoading && <p className="text-[11px] font-mono text-text-muted text-center py-4">Loading chats…</p>}
          ...
          {threads.map(t => (
            <ThreadItem key={t.id} thread={t} active={t.id === activeThreadId} onSelect={openThread} onDelete={handleDelete} />
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-bg-border bg-bg-surface/90 backdrop-blur">
          <button className="lg:hidden p-1.5 rounded-lg hover:bg-bg-muted text-text-muted" onClick={() => setSidebarOpen(true)}>
            <Menu size={16} />
          </button>
          ...
          {threadLoading ? (
            <div className="flex justify-center py-10">
              <div className="flex gap-1">
                {[0,1,2].map(i => <div key={i} className="w-2 h-2 rounded-full bg-accent-gold/50 animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
              </div>
            </div>
          ) : isEmpty ? (
            <div className="max-w-2xl mx-auto">
              <div className="text-center mb-10">
                ...
                <h2 className="text-xl font-display font-semibold text-text-primary mb-2">Your AI UPSC Mentor</h2>
                <p className="text-sm text-text-muted font-mono">Remembers your progress & goals</p>
                {/* Show prefill hint if coming from a quote */}
                {input && (
                  <div className="mt-4 px-4 py-2.5 rounded-xl text-xs font-mono text-text-muted border border-bg-border bg-bg-muted text-left">
                    Ready to discuss the quote — press Enter or click Send ↓
                  </div>
                )}
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {STARTER_QUESTIONS.map((q, i) => (
                  <button key={i} onClick={() => send(q)}
                    ...
                    style={{ borderColor: "var(--bg-border)" }}>{q}</button>
                ))}
              </div>
            </div>
          ) : (
        ...
        <div className="shrink-0 border-t border-bg-border bg-bg-surface/90 backdrop-blur px-4 py-3">
          <div className="max-w-3xl mx-auto">
            <div className="flex gap-2 items-end rounded-2xl border px-3 py-2 transition-colors focus-within:border-accent-gold/40"
              ...
              <textarea ref={textareaRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
                placeholder={contextHint ? `Ask about ${contextHint}…` : "Ask your UPSC mentor…"} rows={1}
                className="flex-1 bg-transparent text-sm text-text-primary focus:outline-none placeholder:text-text-muted resize-none"
              ...
              <button onClick={() => send()} disabled={!input.trim() || sending}
                ...
                style={{ background: "var(--accent-gold)", color: "var(--bg-base)" }}>
                {sending ? <Cpu size={13} className="animate-spin" /> : <ArrowUp size={13} />}
              </button>
            </div>
    ...
    <div className="flex gap-3">
      ...
      <div className="px-4 py-3 rounded-2xl rounded-tl-sm bg-bg-muted border border-bg-border">
        <div className="flex gap-1 items-center h-4">
          {[0,1,2].map(i => <div key={i} className="w-1.5 h-1.5 rounded-full bg-accent-gold/60 animate-bounce" style={{ animationDelay: `${i*0.15}s` }} />)}
        </div>
      </div>
    </div>
  );
