...
// ─── Extracted text (handwritten submissions only) ───────────────────────────
function ExtractedTextPanel({ text }) {
    ...
    return (
        <div>
            <button onClick={() => setOpen(v => !v)}
                className="w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 border"
                style={{
                    background: open ? "var(--accent-blue-dim, rgba(59,130,246,.08))" : "var(--bg-muted)",
                    borderColor: open ? "rgba(59,130,246,.3)" : "var(--bg-border)",
                }}>
                <div className="flex items-center gap-2 flex-wrap">
                    <ScanText size={13} style={{ color: "var(--accent-blue, #3b82f6)" }} />
                    <span className="text-sm font-semibold text-text-primary">Extracted Text</span>
                    <span className="text-[10px] font-mono px-1.5 py-0.5 rounded"
                        style={{ background: "var(--accent-blue-dim, rgba(59,130,246,.1))", color: "var(--accent-blue, #3b82f6)" }}>
                        verify before trusting the score
                    </span>
                </div>
                <ChevronDown size={14} className={`text-text-muted transition-transform duration-300 shrink-0 ${open ? "rotate-180" : ""}`} />
            </button>
            {open && (
                <div className="glass-panel mt-2 p-4 max-h-72 overflow-y-auto">
                    {text.split("\n").filter(Boolean).map((para, i) => (
                        <p key={i} className="text-sm text-text-secondary leading-7 mb-2 last:mb-0 whitespace-pre-wrap">{para}</p>
                    ))}
                </div>
            )}
        </div>
...
function ImageDropzone({ onFile }) {
    ...
    return (
        <div
            ...
            className="flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl py-9 px-4 cursor-pointer transition-colors active:scale-[0.99] touch-manipulation"
...
export default function AIEvaluatorPanel({ question, paper, answer, isLoggedIn }) {
    ...
    const handleEvaluateImage = useCallback(async () => {
        ...
        setError(null); setResult(null); setUnreadable(false); setLoading(true);
        try {
            const res = await evaluateAnswerImage({ question, paper: paper || "GS2", image: imageDataUri });
            setResult(res.data);
            setProvider(res.provider_used);
        } catch (e) {
    ...
    return (
        <div className="mt-4">
            ...
            {!result && !loading && (
                <>
                    {/* Write Answer / Upload Handwritten toggle */}
                    <div className="flex bg-bg-muted rounded-xl p-1 mb-4 border border-bg-border">
                        {[
                            { id: "text", label: "Write Answer", icon: PenLine },
                            { id: "image", label: "Upload Handwritten", icon: Camera },
                    ...
                    {error && unreadable ? (
                        <div className="flex flex-col items-center gap-2 mb-4 px-4 py-5 rounded-xl text-center"
                            ...
                            <button onClick={clearImage} className="btn-ghost flex items-center gap-1.5 text-xs mt-1">
                                <Camera size={12} /> Try a clearer photo
                            </button>
                        </div>
                    ) : error && (
                        <div className="flex items-center gap-2 mb-3 px-3 py-2 rounded-xl bg-red-500/10 border border-red-500/20">
                    ...
                    {mode === "text" && (
                        <>
                            ...
                            <button onClick={handleEvaluate} disabled={wordCount < 5}
                                className="btn-primary flex items-center justify-center gap-2 w-full touch-manipulation">
                                <Sparkles size={13} /> Evaluate Answer with AI
                            </button>
                            <p className="text-[10px] font-mono text-text-muted text-center mt-1.5">
                                Uses Gemini 2.5 Flash · GPT-4o · Claude · Groq (auto-fallback) · 5 evaluations/day
            ...
            {loading && (
                <div className="glass-panel p-6 flex flex-col items-center gap-3">
                    ...
                    {mode === "image" ? (
                        <>
                            <p className="text-sm font-semibold text-text-primary">Reading your handwriting…</p>
                            <p className="text-[11px] font-mono text-text-muted">Transcribing, then evaluating with Gemini Vision</p>
                        </>
                    ) : (
                        <>
                            <p className="text-sm font-semibold text-text-primary">Mentor is analysing…</p>
