...
export default function PrelimsGrind() {
  ...
  const getOfficialLink = (year) => getPrelimsPaperLink(year, activePaper);
        ...
        <header className="rounded-3xl border border-bg-border bg-bg-surface/90 p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-accent-gold">Prelims Grind</p>
              <h1 className="mt-1 font-display text-2xl font-bold text-text-primary">
                Paper-wise prelims drill — PYQs, notes, and NCERT.
              </h1>
              <ul className="list-disc list-inside mt-2 text-sm text-text-secondary space-y-1">
                <li>Pick a paper below — GS Paper I or CSAT — to open its full workspace.</li>
                <li>Use the Resources tab for NCERT, notes, reference books, and YouTube classes.</li>
                <li>Focus on understanding concepts; use My Questions to add your own practice Qs.</li>
              </ul>
            </div>
            <div className="rounded-2xl border border-accent-gold/20 bg-accent-gold/10 px-3 py-2 text-xs text-text-secondary">
              <span className="font-semibold text-accent-gold">Verified:</span>{" "}
        ...
        <section className="rounded-3xl border border-bg-border bg-bg-surface/90 p-5 shadow-sm">
          <div className="mt-2 grid gap-4 md:grid-cols-2">
            {PAPER_OPTIONS.map((paper, index) => {
              const paperSavedCount = Object.keys(savedLinks || {}).filter(k => k.startsWith(paper.id)).length;
              return (
                <button
                  key={paper.id}
                  type="button"
                ...
                >
                  ...
                  <div className="mt-4 flex items-center justify-between rounded-2xl border border-bg-border bg-bg-muted/70 px-3 py-2 text-xs text-text-secondary">
                    <span>
                      {paperSavedCount > 0 ? `${paperSavedCount} saved links` : "Access Workspace"}
                    </span>
                    <span className={`h-2.5 w-2.5 rounded-full ${paperSavedCount > 0 ? "bg-emerald-500" : "bg-amber-500"}`} />
                  </div>
                </button>
              );
            })}
