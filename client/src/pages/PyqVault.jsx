...
import { Archive, Link2, Sparkles, BookOpen, Zap, Library, PenSquare } from "lucide-react";
...
export default function PyqVault({
  ...
  // Main tabs: "practice" or "vault"
  const [mainTab, setMainTab] = useState("practice");
  // Practice sub‑tabs: "browse" or "revision"
  const [practiceSubTab, setPracticeSubTab] = useState("browse");
  // Exam type
  ...
  // PDF Vault state
  const [activeVaultSection, setActiveVaultSection] = useState("prelims");
  const [mainsPaper, setMainsPaper] = useState("gs1");
  const [vaultYearFilter, setVaultYearFilter] = useState("all");
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 space-y-6 font-sans text-text-primary animate-fade-in">
      ...
      <div className="flex w-full border border-bg-border rounded-xl overflow-hidden shadow-sm">
        {[
          ...
          { id: "vault", label: "PDF Vault", icon: Archive },
        ].map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setMainTab(id)}
            className={`flex-1 min-h-[44px] border-none cursor-pointer transition font-sans text-sm flex items-center justify-center gap-1.5 ${
              mainTab === id
      ...
      {mainTab === "vault" && (
        <section className="rounded-3xl border border-bg-border bg-bg-surface/90 p-5 shadow-sm space-y-6">
          <div className="flex gap-2 border-b border-bg-border pb-2">
            {[
              { id: "prelims", label: "Prelims Vault" },
              { id: "mains", label: "Mains Vault" },
            ].map(({ id, label }) => (
              <button
                key={id}
                type="button"
                onClick={() => setActiveVaultSection(id)}
                className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition ${
                  activeVaultSection === id
          ...
          <PdfVaultContent
            activeVaultSection={activeVaultSection}
            mainsPaper={mainsPaper}
            setMainsPaper={setMainsPaper}
            yearFilter={vaultYearFilter}
            setYearFilter={setVaultYearFilter}
          />
        </section>
      )}
