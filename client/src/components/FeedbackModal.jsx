...
export default function FeedbackModal({ isLoggedIn, user }) {
  ...
  // ── Increment counters ──────────────────────────────────────────────────
  // Call these from the respective features:
  // window.incrementFeedbackCounter('notes_auditor')
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        metadata: { useful: form.useful, confusing: form.confusing, improvement: form.improvement },
      };
          ...
          {step === 0 ? (
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex gap-1.5">
                  {[1, 2, 3, 4, 5].map((r) => (
                    ...
                    >
                      <Star
                        size={20}
                        fill={form.rating >= r ? "currentColor" : "none"}
                      />
                    </button>
                  ))}
                </div>
                ...
                <div className="flex gap-3">
                  {["Yes", "No", "Maybe"].map((opt) => (
                    <button
                      key={opt}
                      type="button"
                      onClick={() => setForm((prev) => ({ ...prev, wouldRecommend: opt === "Yes" ? true : opt === "No" ? false : null }))}
              ...
              <label className="flex items-center gap-2 text-sm text-text-secondary cursor-pointer">
                <input
                  ...
                  onChange={(e) => setForm((prev) => ({ ...prev, allowReply: e.target.checked }))}
            ...
            <div className="text-center py-8 space-y-3">
              ...
              <p className="text-sm text-text-secondary max-w-sm mx-auto">
                Your input helps us make UPSC Mentor better for everyone.
